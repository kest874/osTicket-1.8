...
<div class="merge-tickets" style="overflow-y: auto;">
<?php
if ($tickets) {
...
<li class="<?php if ($isLinkChild) echo 'sortable'; ?> row-item
    ...
    echo sprintf('%s %s %s %s %s <div style="position:absolute; right:40px; margin-top: -24px;">%s %s %s %s %s</div>',
...
<button type="button" class="btn btn-sm btn-outline-secondary" onclick="javascript:
    ids = [];
    ...
    if (!inOriginalIds) {
        $('.merge-tickets').append($('<li></li>').addClass('sortable row-item')
            ...
            .append(spaces)
            .append($('<div style=\'position:absolute; right:40px;margin-top: -24px;\'></div>')
                .append(showEntries)
...
<?php echo __('Add Selected Ticket'); ?></button>
</div>
<div>
    <hr>
...
<div id="merge-options">
     <div class="custom-control custom-checkbox">
        <input type="checkbox" class="custom-control-input" id="delete-child" name="delete-child">
        <label class="custom-control-label" for="delete-child"><?php echo __('Delete Child Ticket') ?></label>
			</div>
			<div class="custom-control custom-checkbox">
        <input type="checkbox" class="custom-control-input" id="move-tasks" name="move-tasks">
        <label class="custom-control-label" for="move-tasks"><?php echo __('Move Child Tasks to Parent') ?></label>
			</div>
</div>
...
<div>
    <span class="buttons pull-left">
        <input type="button" name="cancel" class="btn btn-danger btn-sm <?php
            echo $user ? 'cancel' : 'close' ?>" value="<?php echo __('Cancel'); ?>">
    </span>
    <?php if (!$info['error']) { ?>
        <span class="buttons pull-right">
            <input type="submit" class="btn btn-primary btn-sm" value="<?php echo __('Save Changes'); ?>">
        </span>
    <?php } ?>
 </div>
